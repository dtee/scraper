...
Public Class TagTreeView
   ...
   ''' <returns></returns>
   ''' <remarks></remarks>
   Private Function IsValidNode(ByVal aNode As TagNode) As Boolean
      Dim isValid As Boolean = aNode.IsValid

      For Each n As TagNode In aNode.Nodes
   ...
   Public Property [TagTree]() As TagTree
      ...
      Set(ByVal value As TagTree)
         ...
         If value IsNot Nothing Then
            ...
            _RootNode = New TagNode(_TagTree.RootTag, Me)

            MyBase.Nodes.Add(_RootNode)
            RecursiveLoadNodes(_RootNode)
   ...
   ''' <summary>
   ''' Recursively Load Node using dataview...
   ''' </summary>
   ''' <param name="node">load to recursively load</param>
   ''' <remarks></remarks>
   Private Sub RecursiveLoadNodes(ByVal node As TagNode)
      Dim dv As New DataView(node.Row.Table)
      dv.RowFilter = String.Format("Parent_TagID = {0}", node.TagID)
      dv.Sort = "OrderNumber"

      For i As Integer = 0 To dv.Count - 1
         Dim row As ScraperDB.TagLibraryRow = dv.Item(i).Row

         Dim newNode As New TagNode(row, Me)
         node.Nodes.Add(newNode)
         newNode.CalculteImageIndex()

         RecursiveLoadNodes(newNode)
      Next
   End Sub
   ...
   ''' </remarks>
   Public Function CreateNode(ByVal name As String, ByVal anode As TagNode, Optional ByVal isBelow As Boolean = False) As TagNode
      Dim tNode As TagNode
      If isBelow Then
         Dim pnode As TagNode = anode.Parent
         Dim row As ScraperDB.TagLibraryRow = Me._TagTree.CreateTag(name, pnode.Row)

         tNode = New TagNode(row, Me)
         ...
         End While
      Else
         Dim row As ScraperDB.TagLibraryRow = Me._TagTree.CreateTag(name, anode.Row)
         tNode = New TagNode(row, Me)
   ...
   ''' Raiseevent - TreeChanged
   ''' </remarks>
   Public Sub DeleteNode(ByVal node As TagNode)
      node.Remove()
   ...
   ''' <returns></returns>
   ''' <remarks></remarks>
   Public Function MoveNode(ByVal destNode As TagNode, ByVal sourceNode As TagNode) As Boolean
      ' Change the order
   ...
   ''' <remarks></remarks>
   Public Function IsDropAllowed(ByVal destNode As TagNode, ByVal sourceNode As TagNode) As Boolean
      Dim tnCurrent As TagNode = destNode
   ...
   Public Sub MoveNode(ByVal targetNode As TagNode, ByVal op As NodeMovement)
      Dim pNode As TagNode = targetNode.Parent
      'Dim nodeQueue As New Queue(pNode.LastNode.Index + 1)

      Dim nodeQueue As New LinkedList(Of TagNode)

      Dim checknode As TagNode = targetNode.PrevNode
      ...
      For Each tNode As TagNode In nodeQueue
         pNode.Nodes.Add(tNode)
         tNode.UpdateData()
      Next
   ...
   ''' </remarks>
   Private Sub UpdateOrder(ByVal node As TagNode)
   ...
   ''' <remarks></remarks>
   Public Overloads Property SelectedNode() As TagNode
      Get
   ...
   Private _OldSelectedNode As TagNode
   ''' <summary>
   ...
   ''' </remarks>
   Public Property TempSelectedNode() As TagNode
      Get
         Return MyBase.SelectedNode
      End Get
      Set(ByVal value As TagNode)
